---
slug: 2026-05-18-techniek-bc
datum: 2026-05-18
seizoen: 2025-2026
periode: Wedstrijdspecifiek
primair_thema: Techniek
secundair_thema: Wedstrijdtempo
sessievorm: 2 blokken
slagfocus: "Techniek: RC; Wedstrijdtempo: BC"
totale_afstand_m: 2600
duur_min: 60
public: true
---

# Training

## Inzwemmen - 600m

- `1x300m` `R:0:20` `70%` - `100 BC` > `100 WIS` > `100 BC`
- `2x100m` `R:0:15` `70%` - `25 benen RC` > `50 armen RC` > `25 RC`
- `2x50m` `R:0:15` `75%` - `25 RC` > `25 BC`, soepel doortrekken

## Blok 1 - Techniek - RC - 700m

- `4x50m` `25 RC 6 beenslagen zijligging` > `25 RC` - stabiele zijligging voor elke armactie {{coach: laat de heup hoog blijven en voorkom dat het hoofd meedraait}}
- `4x50m` `25 RC enkelvoudige arm links/rechts` > `25 RC` - per herhaling wisselen van werkarm {{coach: let op rechte insteek naast de schouder en rustige herstelarm}}
- `2x100m` `25 RC benen` > `25 RC armen` > `50 RC` `R:0:20` `70%` - techniek opbouwen naar volledige slag {{coach: bewaak dat de armdoorhaal pas start na een stabiele ligging}}
- `2x50m` `RC` `R:0:15` `70%` - toepassen met lange lijn {{coach: tel slagen per baan en houd het ritme rustig}}

## Blok 2 - Wedstrijdtempo - BC - 1300m

- `6x50m` `BC` `R:0:20` `95%` - wedstrijdtempo met vaste slaglengte {{coach: stuur op tempo uit ritme en niet uit korter zwemmen}}
- `2x100m` `BC` `R:0:25` `70%` - herstel met technische controle {{coach: laat de zwemmers lengte direct terugvinden na tempo}}
- `4x100m` `BC` `R:0:25` `95%` - eerste `50m` wedstrijdtempo, tweede `50m` `75%` strak door {{coach: bewaak vlakke houding en ontspannen ademhaling in de tweede helft}}
- `4x50m` `BC` `R:0:25` `95%` - breakout tot `15m`, daarna ritme vasthouden {{coach: kijk of de eerste armslag pas komt na volledige versnelling}}
- `2x100m` `SNK` `R:0:20` `50%` - herstel met snorkel {{coach: laat hartslag zakken en schouders ontspannen}}
